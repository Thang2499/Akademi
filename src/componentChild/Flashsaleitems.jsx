 ...
 const getProductDetail = (item) =>{
  ...
  return (
    <div className='flex flex-col w-52 ml-8 relative group'>
     ...
     <Link onClick={()=>getProductDetail(item)} to="/productDetail"> <div className='font-medium' >{productName}</div></Link>
      <div className='flex'>
      ...
      <img
        className={` absolute bg-white right-2 top-1 rounded-xl  cursor-pointer ${isWishlistActive || favorites.includes(id) ? 'bg-cyan-400 filter-white' : 'bg-white'}`}
        src={Wishlist}
        alt="Wishlist"
